...
CurveWindow::CurveWindow (int width, int height, plug::Vec2d lh_pos, const char *w_name, 
    ...
    apply_button  (plug::Vec2d (0, 200), 60, 30, "APPLY", 
                  ...
                  new std::pair<Curve_filter *, CurveTool *> (curve_filter, curve_tool), PRESS_BUTTON),
...
CurveWindow::~CurveWindow ()
{
    ...
    {
        delete (std::pair<Curve_filter *, CurveTool *> *)apply_button.get_arg ();
        apply_button.set_arg (nullptr);
    }
    if (cancel_button.get_arg ())
    {
        delete  (std::pair<Curve_filter *, CurveTool *> *)cancel_button.get_arg ();
        cancel_button.set_arg (nullptr);
    }
    if (cancel_button.get_widget ())
    {
        delete (std::pair<Window * ,plug::Canvas *> *)cancel_button.get_widget ();
        cancel_button.set_widget (nullptr);
    }
    if (reset_button.get_arg ())
    {
        delete  (std::pair<Curve_filter *, CurveTool *> *)reset_button.get_arg ();
